...
- **夜 22:00** — 一日を振り返って日記を書き、エネルギーを下げます。Discord Webhook が設定されていれば、スナップショット画像も自動送信します
...
### 14-2. OpenClaw のセキュリティ強化（YADORI 専用設定）
...
> **openclaw.json の場所:** `~/.openclaw/openclaw.json`
...
| 設定 | 意味 |
|------|------|
| `gateway.mode: "local"` | Gateway をローカルのみで動作させる |
| `gateway.bind: "loopback"` | 外部ネットワークからの接続を完全に遮断する |
| `tools.allow: ["read", "message"]` | 知性体が使えるツールを「ファイル読み取り」と「メッセージ送受信」だけに制限する |
| `tools.deny: [...]` | コマンド実行、Web ブラウジング、検索、自動化などを明示的に禁止する |
| `tools.fs.workspaceOnly: true` | ファイルアクセスをワークスペース（`~/.openclaw/workspace/`）内に限定する |
| `tools.exec.security: "deny"` | コマンド実行を拒否する |
| `tools.elevated.enabled: false` | 管理者権限のツール実行を無効化する |
| `session.dmScope: "per-channel-peer"` | DM の会話コンテキストをユーザーごとに分離する |
| `skipBootstrap: true` | YADORI が自分でワークスペースを管理しているため、OpenClaw のデフォルトファイル作成をスキップする |
...
1. システム設定 → ユーザーとグループ → ユーザーを追加
2. 「標準」ユーザーとして作成（管理者にしない）
3. そのユーザーでログインし、YADORI をセットアップ

#### macOS ファイアウォール

1. システム設定 → ネットワーク → ファイアウォール → オンにする
2. 「外部からの接続をすべてブロック」は有効にしてください
3. OpenClaw Gateway と YADORI ダッシュボードは localhost のみで動作するため、外部接続を許可する必要はありません
